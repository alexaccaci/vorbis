...
class Drft {
  int n;
  var trigcache = List<double>();
  var splitcache = List<int>();

  void backward(List<double> data) {
    if (n == 1) return;
  ...
  void init(int n) {
    this.n = n;
    trigcache = List<double>(3 * n);
    splitcache = List<int>(32);
  ...
  static void drfti1(int n, List<double> wa, int index, List<int> ifac) {
    double arg, argh, argld, fi;
    int ntry = 0, i, j = -1;
    int k1, l1, l2, ib;
    int ld, ii, ip, ix, nq, nr;
    int ido, ipm, nfm1;
    int nl = n;
    int nf = 0;
  ...
  static void fdrffti(int n, List<double> wsave, List<int> ifac) {
    if (n == 1) return;
    drfti1(n, wsave, n, ifac);
  }

  static void dradf2(int ido, int l1, List<double> cc, List<double> ch,
      List<double> wa1, int index) {
    int i, k;
    double ti2, tr2;
  ...
  static void drftf1(
      int n, List<double> c, List<double> ch, List<double> wa, List<int> ifac) {
    int i, k1, l1, l2;
    int na, kh, nf;
  ...
  static void dradb2(int ido, int l1, List<double> cc, List<double> ch,
      List<double> wa1, int index) {
    int i, k, t0, t1, t2, t3, t4, t5, t6;
    double ti2, tr2;
  ...
  static void dradb3(int ido, int l1, List<double> cc, List<double> ch,
      List<double> wa1, int index1, List<double> wa2, int index2) {
    ...
    if (ido != 2) {
      ...
      for (k = 0; k < l1; k++) {
        ...
        for (i = 2; i < ido; i += 2) {
          ...
          ch[(t8 = t7 + t0) - 1] =
              wa1[index1 + i - 2] * cr2 - wa1[index1 + i - 1] * ci2;
          ch[t8] = wa1[index1 + i - 2] * ci2 + wa1[index1 + i - 1] * cr2;
          ch[(t8 += t0) - 1] =
              wa2[index2 + i - 2] * cr3 - wa2[index2 + i - 1] * ci3;
          ch[t8] = wa2[index2 + i - 2] * ci3 + wa2[index2 + i - 1] * cr3;
          ch[(t8 += t0) - 1] =
              wa3[index3 + i - 2] * cr4 - wa3[index3 + i - 1] * ci4;
  ...
  static void drftb1(int n, List<double> c, List<double> ch, List<double> wa,
      int index, List<int> ifac) {
    int i, k1, l1, l2 = 0;
    int na;
      ...
      while (loop) {
        switch (state) {
          case 100:
            ...
            if (na != 0)
              dradb4(ido, l1, ch, c, wa, index + iw - 1, wa, index + ix2 - 1,
                  wa, index + ix3 - 1);
            else
              dradb4(ido, l1, c, ch, wa, index + iw - 1, wa, index + ix2 - 1,
                  wa, index + ix3 - 1);
            na = 1 - na;
            state = 115;
            break;
